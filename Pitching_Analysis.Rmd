...
without hyperparameter tuning. Credit to two articles that provided tactical
guidance on how best to work with XGBoost: 

- [1](https://xgboost.readthedocs.io/en/latest/R-package/xgboostPresentation.html)
- [2](https://towardsdatascience.com/getting-to-an-hyperparameter-tuned-xgboost-model-in-no-time-a9560f8eb54b)

```{r xgboost}
###### REMOVE WHEN DONE TROUBLESHOOTING
pitcher_name <- "R.A. Dickey"
year <- 2017
...
pitch_classify <- function(pitcher_name, year = 2020) {
  ...
  filter_data <- braves_pitch %>% 
    filter(player_name == pitcher_name, game_year == year,
           ...
           !is.null(pitch_name), !is.na(pitch_name)) %>% 
...
pitch_classify(pitcher_name = "R.A. Dickey", year = 2017)
...
- Figure out how to filter out NULL values (ask Marc)
- Perform cross-validation and/or hyper-parameter tuning?
- Report on results for a few pitchers of interest
- Investigate areas of systematic (or even sporadic) underperformance
- Write a README
